...
const Card = ({
  ...
  return (
    <div className="py-8">
      ...
      {/* Disconnected state for My Campaigns */}
      {isMyTab && !address ? (
        <div className="glass-panel rounded-3xl p-12 text-center border border-white/10 max-w-lg mx-auto">
          ...
          <p className="text-sm text-gray-400 mb-6">
            Please connect your MetaMask wallet to view and manage campaigns you have created.
          </p>
          <button
            onClick={connectWallet}
            disabled={isLoading}
            className="px-6 py-3 rounded-xl text-xs font-semibold text-white gradient-btn shadow-lg"
          >
          ...
          {allCampaign.length === 0 ? (
            <div className="col-span-full glass-panel rounded-3xl p-12 text-center border border-white/5">
              ...
              <p className="text-sm text-gray-400 max-w-md mx-auto mb-6">
                {isMyTab
                  ? "You have not launched any campaigns yet. Create your first project today!"
                  : "There are no campaigns matching your current filter. You can be the first to start a project."}
              </p>
              {onOpenCreateModal && (
                <button
